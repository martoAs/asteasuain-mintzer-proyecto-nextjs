...
export default async function handler(req: { method: string; items: Items[]; }, res: { json: (arg0: { id: string | undefined; }) => void; status: (arg0: number) => { (): any; new(): any; send: { (arg0: { error: string; }): void; new(): any; }; }; }){
    if(req.method == 'POST'){
        try{
            ...
            if(token) {
                ...
                const body = {
                    body:{
                        items : await validateCart(req.items),
                        back_urls: {
